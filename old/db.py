...
def ask_object(client: Client, source_path: str, search_string: str, question: Optional[str] = None) -> (str, Dict):
    ...
    summmary_response = (
        client.query.get(CollectionName.SOURCE.value, "body")
        .with_where(source_filter(source_path))
        .with_limit(1)
        .do()
    )
    summary = summmary_response["data"]["Get"][CollectionName.SOURCE.value][0]["body"]

    print("Getting chunks")
    chunks_response = (
        client.query.get(CollectionName.CHUNK.value, "body")
        .with_where(source_filter(source_path))
        .with_near_text({"concepts": search_string})
        .with_limit(MAX_N_CHUNKS)
        .do()
    )
    chunks = [c["body"] for c in chunks_response["data"]["Get"][CollectionName.CHUNK.value]]
